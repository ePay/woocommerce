...
class Bambora_Online_Classic_Helper
{
    ...
    const BAMBORA_ONLINE_CLASSIC_STATUS_MESSAGES = 'bambora_online_classic_status_messages';
    const BAMBORA_ONLINE_CLASSIC_STATUS_MESSAGES_KEEP_FOR_POST = 'bambora_online_classic_status_messages_keep_for_post';
    const ERROR = 'error';
    ...
    public static function get_bambora_online_classic_subscription_id($subscription)
    {
        ...
        $bambora_subscription_id = get_post_meta(
            $subscription_id,
    ...
    public static function get_bambora_online_classic_transaction_id($order)
    {
        ...
        if (empty($transaction_id)) {
            ...
            $transaction_id = get_post_meta(
                $order_id,
                self::BAMBORA_ONLINE_CLASSIC_TRANSACTION_ID_LEGACY,
                true
            );
            if (!empty($transaction_id)) {
                //Transform Legacy to new standards
     ...
     * @return bool
     */
    public static function validate_bambora_online_classic_callback_params(
        $params,
        $md5_key,
        &$order,
        &$message
        ...
        if (class_exists('sitepress')) {
            $order_language = Bambora_Online_Classic_Helper::getWPMLOrderLanguage(
                $order
            );
            $md5_key = Bambora_Online_Classic_Helper::getWPMLOptionValue(
                'md5key',
                $order_language,
                $md5_key
            );
    ...
    public static function add_admin_notices($type, $message, $keep_post = false)
    {
        ...
        if (!$messages) {
            update_option(
                self::BAMBORA_ONLINE_CLASSIC_STATUS_MESSAGES,
                array($message)
            );
        } else {
            array_push($messages, $message);
            update_option(self::BAMBORA_ONLINE_CLASSIC_STATUS_MESSAGES, $messages);
        }
        update_option(
            self::BAMBORA_ONLINE_CLASSIC_STATUS_MESSAGES_KEEP_FOR_POST,
            $keep_post
        );
    }
    ...
    public static function echo_admin_notices()
    {
        ...
        foreach ($messages as $message) {
            echo Bambora_Online_Classic_Helper::message_to_html(
                $message['type'],
                $message['message']
            );
        }
